        ...
        while True:
            ...
            if choice == '1':
                ...
                if not courses:
                    ...
                    print(" CourseID    Class     Subject                  Year  Sem  Teacher")
                    print("-" * 80)
                    for c in courses:
                        teacher = f"{c['TeacherName'] or 'Not assigned'}"[:25]
                        print(f" {c['CourseID']:<11} {c['ClassName']:<9} {c['SubjectName'][:20]:<20} {c['Year']}  {c['Semester']}  {teacher}")
                input("\nPress Enter to continue...")
            ...
            if choice == '2':
                ...
                if teachers:
                    ...
                    sel = input("Choose teacher sequence number (Enter to skip): ").strip()
                    if sel.isdigit():
                        try:
                            idx = int(sel) - 1
                ...
                confirm = input(f"\nConfirm adding class: {course_id} - {class_name} ({year} Sem{semester}) - Capacity: {class_size} - Description: {description or 'None'}? (Y/N): ").upper()
                if confirm == 'Y':
                    try:
                        self.cursor.execute("""
                ...
                print(f"\n--- UPDATE COURSE SECTION (subject {selected_subject_id}) ---")
                course_list = self.cursor.execute("""
                    SELECT CourseID, ClassName, Year, Semester, TeacherID, ClassSize, Description
                    FROM Course 
                    WHERE SubjectID = ?
                ...
                if not course_list:
                    print("No course sections for this subject yet.")
                    input("Press Enter...")
                    continue
                ...
                try:
                    idx = int(input("\nChoose class sequence number to edit: ")) - 1
                    if idx < 0 or idx >= len(course_list):
                        raise ValueError
                ...
                print("\nCurrent information:")
                print(f"  Class code  : {course_id}")
                print(f"  Class name  : {selected_course['ClassName']}")
                print(f"  Year - Sem  : {selected_course['Year']} - {selected_course['Semester']}")
                print(f"  Teacher     : {selected_course['TeacherID'] or 'Not assigned'}")
                print(f"  Capacity    : {selected_course['ClassSize']}")
                print(f"  Description : {selected_course['Description'] or 'None'}")

                new_class_name = input(f"New class name (Enter to keep): ").strip() or selected_course['ClassName']

                year_input = input(f"New academic year (Enter to keep {selected_course['Year']}): ").strip()
                new_year = int(year_input) if year_input else selected_course['Year']

                sem_input = input(f"New semester (Enter to keep {selected_course['Semester']}): ").strip()
                new_semester = int(sem_input) if sem_input else selected_course['Semester']

                # Update capacity
                class_size_input = input(f"New capacity (Enter to keep {selected_course['ClassSize']}): ").strip()
                new_class_size = int(class_size_input) if class_size_input else selected_course['ClassSize']

                # Update description
                new_description = input(f"New description (Enter to keep): ").strip() or selected_course['Description']

                # Update teacher (optional)
                new_teacher_id = selected_course['TeacherID']
                if input("Change teacher? (Y/N): ").upper() == 'Y':
                    teachers = self.cursor.execute("SELECT TeacherID, FullName FROM Teacher t JOIN User u ON t.AccountID=u.AccountID").fetchall()
                    if teachers:
                        for i, t in enumerate(teachers, 1):
                            print(f" {i}. {t['TeacherID']} - {t['FullName']}")
                        sel = input("Choose new teacher (Enter to keep): ").strip()
                        if sel.isdigit():
                            try:
                                idx_t = int(sel) - 1
                                new_teacher_id = teachers[idx_t]['TeacherID']
                            except:
                                pass
                ...
                if confirm == 'Y':
                    try:
                        self.cursor.execute("""
                            UPDATE Course 
                            SET ClassName = ?, Year = ?, Semester = ?, TeacherID = ?, ClassSize = ?, Description = ?
                            WHERE CourseID = ?
                        """, (new_class_name, new_year, new_semester, new_teacher_id, new_class_size, new_description, course_id))
                        self.conn.commit()
                ...
                print(f"\n--- DELETE COURSE SECTION (subject {selected_subject_id}) ---")
                course_list = self.cursor.execute("SELECT CourseID, ClassName FROM Course WHERE SubjectID = ?",
                                                  (selected_subject_id,)).fetchall()
                ...
                for i, c in enumerate(course_list, 1):
                    print(f" {i}. {c['CourseID']} - {c['ClassName']}")
                try:
                    idx = int(input("\nChoose class sequence number to delete: ")) - 1
                    if idx < 0 or idx >= len(course_list):
                        raise ValueError
                    selected_course_id = course_list[idx]['CourseID']
                except:
                    print("[Error]: Invalid choice!")
                    input("Press Enter...")
                    continue

                # Check constraint: any students registered?
                enroll_count = self.cursor.execute("SELECT COUNT(*) FROM Enrollment WHERE CourseID = ?",
                                                   (selected_course_id,)).fetchone()[0]
                if enroll_count > 0:
                    print(f"[Error]: Cannot delete! This class has {enroll_count} registered students.")
                    input("Press Enter...")
                    continue

                confirm = input(f"Confirm DELETE class {selected_course_id}? (Y/N): ").upper()
                if confirm == 'Y':
                    try:
                        self.cursor.execute("DELETE FROM Course WHERE CourseID = ?", (selected_course_id,))
                        self.conn.commit()
...
# 5. Add student to class (new function)
            elif choice == '5':
                ...
                subjects = self.cursor.execute("SELECT SubjectID, SubjectName FROM Subject ORDER BY SubjectID").fetchall()
                if not subjects:
                    print("[Error]: No subjects in the system yet!")
                    input("Press Enter...")
                    continue
                ...
                idx_input = input("\nEnter subject sequence number (Enter to return): ").strip()
                if not idx_input:
                ...
                if not courses:
                    print(f"[Notification]: Subject {selected_subject_id} has no classes yet.")
                    input("Press Enter...")
                    continue
            ...
            while True:
                ...
                try:
                    if choice == '2':
                        ...
                        # Display current schedules to select
                        ...
                        if not scheds:
                            print("This class has no schedules yet. Added new above if confirmed.")
                            break
                        ...
                        try:
                            ...
                            print("[Error]: Invalid choice!")
                            continue  # back to ask all info again
    ...
    def view_reports(self):
        """Use-case 15: Statistical Reports - Detailed system statistical report (added pass/fail subjects)"""
        self.clear_screen()
        print("====================================")
        print("   SYSTEM STATISTICAL REPORT")
        print("====================================")
